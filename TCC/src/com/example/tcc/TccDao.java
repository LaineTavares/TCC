...
public class TccDao {
	...
	public static final String CREATE_TABLE_ANIMAL = "CREATE TABLE IF NOT EXISTS "
			...
			+ cor + " TEXT, "+ idTabelaTipoAnimal + " Long);";
	...
	public static final String CREATE_TABLE_INGREDIENTE = "CREATE TABLE IF NOT EXISTS "
			+ tabela_ingrediente + "("+ tabela_ingrediente  
			+ " INTEGER PRIMARY KEY AUTOINCREMENT," + precoIngrediente + " TEXT ," + nomeIngrediente + " TEXT );";
	...
	public static final String CREATE_TABLE_INGREDIENTE_NUTRIENTE = "CREATE TABLE IF NOT EXISTS "
			+ tabela_ingrediente_nutriente + "("+ idTabelaIngredienteNutriente  
			+ " INTEGER PRIMARY KEY AUTOINCREMENT," + idIngre_Nutri + " Long ," + porcentagem_Nutri +" TEXT ," 
			+ idNutri_Ingre + " LONG );";
	...
	public static final String CREATE_TABLE_TIPO_ANIMAL = "CREATE TABLE IF NOT EXISTS "
			+ tabela_tipo_animal + "("+ idTabelaTipoAnimal  
	...
	private ContentValues gerarContentValuesAnimais(
			Animal animal) {
		ContentValues values = new ContentValues();
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					String cor = String.valueOf(cursor
							.getString(indexCor));
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					//int indexIdIngrediente = cursor.getColumnIndex(idIngrediente);
					int indexIdTabelaIngrediente = cursor.getColumnIndex(tabela_ingrediente);
					int indexNomeIngrendiente = cursor.getColumnIndex(nomeIngrediente);
					int indexPrecoIngrediente = cursor.getColumnIndex(precoIngrediente);
					
				
					int id = cursor.getInt(indexIdTabelaIngrediente);
					String precoIngrediente = cursor.getString(indexPrecoIngrediente);
					String nomeIngrediente = cursor
							.getString(indexNomeIngrendiente);
				
					Ingrediente ingrediente = new Ingrediente(nomeIngrediente, id, precoIngrediente);
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					String nomeNutriente = cursor
							.getString(indexNomeNutriente);
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					int indexIdTipoAnimal = cursor.getColumnIndex(idTabelaTipoAnimal);
					int indexTipoAnimal = cursor.getColumnIndex(tipoAnimal);
					
				
					int id = cursor.getInt(indexIdTipoAnimal);
					String tipoAnimal = cursor
							.getString(indexTipoAnimal);
